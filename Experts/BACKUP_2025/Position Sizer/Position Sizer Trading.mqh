...
void Trade()
{
    ...
    if (sets.Commentary != "")
        Commentary = sets.Commentary;

    if (sets.CommentAutoSuffix)
        Commentary += IntegerToString((int)TimeLocal());
    ...
    if ((sets.MaxNumberOfTradesTotal > 0) || (sets.MaxNumberOfTradesPerSymbol > 0))
    {
        ...
        if (AccountInfoInteger(ACCOUNT_MARGIN_MODE) == ACCOUNT_MARGIN_MODE_RETAIL_HEDGING) // Makes sense in a hedging mode.
        {
            for (int i = 0; i < total; i++)
            {
                if (!PositionSelectByTicket(PositionGetTicket(i)))
                    continue;
                if ((sets.MagicNumber != 0) && (PositionGetInteger(POSITION_MAGIC) != sets.MagicNumber))
                    continue;
                if (PositionGetString(POSITION_SYMBOL) == SymbolForTrading)
                    persymbol_cnt++;
                cnt++;
            }
        }
        else // In netting, it can only count positions on different symbols.
        {
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                if ((sets.MagicNumber != 0) && (PositionGetInteger(POSITION_MAGIC) != sets.MagicNumber))
                    continue;
                if ((PositionGetString(POSITION_SYMBOL) == SymbolForTrading) && (sets.EntryType == Instant))
                    continue; // Skip current symbol, because in netting mode, new trade won't create another position, but will rather add/subtract from the existing one.
                cnt++;
            }
        }
        ...
        for (int i = 0; i < total; i++)
        {
            if (!OrderSelect(OrderGetTicket(i)))
                continue;
            if ((sets.MagicNumber != 0) && (OrderGetInteger(ORDER_MAGIC) != sets.MagicNumber))
                continue;
            if (OrderGetString(ORDER_SYMBOL) == SymbolForTrading)
                persymbol_cnt++;
            cnt++;
        }
    ...
    if (sets.MagicNumber > 0)
        Trade.SetExpertMagicNumber(sets.MagicNumber);
    ...
    if ((Execution_Mode == SYMBOL_TRADE_EXECUTION_MARKET) && (IgnoreMarketExecutionMode) && (sets.EntryType == Instant))
        warning_suffix = TRANSLATION_MESSAGE_IGNORE_MARKET_EXECUTION_MODE_WARNING;
    ...
    if ((sets.EntryType == Pending) || (sets.EntryType == StopLimit))
    {
        ...
        {
            // Stop
            if (sets.EntryLevel < SymbolInfoDouble(SymbolForTrading, SYMBOL_BID))
        ...
        {
            // Stop
            if (sets.EntryLevel > SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK))
        ...
        if ((sets.SubtractPendingOrders) || (sets.SubtractPositions))
        {
            if ((ot == ORDER_TYPE_BUY_LIMIT) || (ot == ORDER_TYPE_BUY_STOP) || (ot == ORDER_TYPE_BUY_STOP_LIMIT))
                PositionSize -= existing_volume_buy;
            else
                PositionSize -= existing_volume_sell;
        ...
        if (new_ps < 0)
            return; // Doesn't conform restrictions and cannot be reduced.
        ...
        {
            if (ArrayPositionSize[j] == 0)
                continue; // Calculated PS < broker's minimum.
            ...
            if ((tp != 0) && (((tp <= sets.EntryLevel) && ((ot == ORDER_TYPE_BUY_STOP_LIMIT) || (ot == ORDER_TYPE_BUY_STOP) || (ot == ORDER_TYPE_BUY_LIMIT))) || ((tp >= sets.EntryLevel) && ((ot == ORDER_TYPE_SELL_STOP_LIMIT) || (ot == ORDER_TYPE_SELL_STOP) || (ot == ORDER_TYPE_SELL_LIMIT)))))
                tp = 0; // Do not apply TP if it is invald. SL will still be applied.
            while (position_size > 0)
            {
                ...
                {
                    if (sets.TakeProfitsNumber == 1)
                        Print(TRANSLATION_MESSAGE_ORDER_EXECUTED + " " + TRANSLATION_MESSAGE_TICKET + ": ", Trade.ResultOrder(), ".");
                    else
                        Print(TRANSLATION_MESSAGE_ORDER + " #", j, " " + TRANSLATION_MESSAGE_EXECUTED + ". " + TRANSLATION_MESSAGE_TICKET + ": ", Trade.ResultOrder(), ".");
    ...
    else
    {
        ...
        for (int j = 0; j < sets.TakeProfitsNumber; j++)
        {
            ...
            if ((order_tp != 0) && (((order_tp <= sets.EntryLevel) && (ot == ORDER_TYPE_BUY)) || ((order_tp >= sets.EntryLevel) && (ot == ORDER_TYPE_SELL))))
                order_tp = 0; // Do not apply TP if it is invald. SL will still be applied.
...
bool ApplySLTPToOrder(CTrade &trade_object, ulong deal, ulong order, double sl, double tp, ENUM_ORDER_TYPE ot)
{
    ...
    if ((tp != 0) && (((tp <= sets.EntryLevel) && (ot == ORDER_TYPE_BUY)) || ((tp >= sets.EntryLevel) && (ot == ORDER_TYPE_SELL))))
        tp = 0; // Do not apply TP if it is invald. SL will still be applied.
    ...
    if (sets.SubtractPositions)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
                volume_buy += PositionGetDouble(POSITION_VOLUME);
            // Short positions
            else if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL)
                volume_sell += PositionGetDouble(POSITION_VOLUME);
        }
    }
}
...
bool CheckConfirmation(const ENUM_ORDER_TYPE ot, const double PositionSize, const double sl, const double tp, const datetime expiry = 0)
{
    ...
    if (sets.TakeProfitsNumber > 1)
        message += " (" + TRANSLATION_MESSAGE_MULTIPLE + ")";
    message += "\n";
    if (sets.AccountButton == Balance)
        message += TRANSLATION_BUTTON_ACCOUNT_BALANCE;
    else if (sets.AccountButton == Equity)
        message += TRANSLATION_BUTTON_ACCOUNT_EQUITY;
    else if (sets.AccountButton == Balance_minus_Risk)
        message += TRANSLATION_BUTTON_BALANCE_MINUS_CPR;
    ...
    if (PositionMargin != 0)
        message += TRANSLATION_TAB_BUTTON_MARGIN + ": " + FormatDouble(DoubleToString(PositionMargin, 2)) + " " + AccountCurrency + "\n";
    if (sets.StopPriceLevel > 0)
        message += TRANSLATION_LABEL_STOPPRICE + ": " + DoubleToString(sets.StopPriceLevel, _Digits) + "\n";
    message += TRANSLATION_LABEL_ENTRY + ": " + DoubleToString(sets.EntryLevel, _Digits) + "\n";
    if (!sets.DoNotApplyStopLoss)
        message += TRANSLATION_LABEL_STOPLOSS + ": " + DoubleToString(sets.StopLossLevel, _Digits) + "\n";
    ...
    if (expiry > 0)
        message += TRANSLATION_LABEL_EXPIRY + ": " + TimeToString(expiry, TIME_DATE | TIME_MINUTES | TIME_SECONDS);
...
void DoTrailingStop()
{
    ...
    for (int i = 0; i < PositionsTotal(); i++)
    {
        ...
        if (ticket <= 0)
            Print(TRANSLATION_MESSAGE_POSITIONGETTICKET_FAILED + ": " + ErrorDescription(GetLastError()) + ".");
        else if (SymbolInfoInteger(PositionGetString(POSITION_SYMBOL), SYMBOL_TRADE_MODE) == SYMBOL_TRADE_MODE_DISABLED)
            continue;
        else
        {
            if ((PositionGetString(POSITION_SYMBOL) != SymbolForTrading) || (PositionGetInteger(POSITION_MAGIC) != sets.MagicNumber))
                continue;
...
void DoBreakEven()
{
    ...
    if (be_line_color != clrNONE)
    {
        ...
        for (int i = obj_total - 1; i >= 0; i--)
        {
            ...
            if (StringFind(obj_name, ObjectPrefix + "BE") == -1)
                continue; // Skip all other horizontal lines.
            ...
            if (!PositionSelectByTicket(ticket)) // No longer exists.
            {
                ...
                if (ShowLineLabels)
                    ObjectDelete(ChartID(), ObjectPrefix + "BEL" + IntegerToString(ticket)); // Delete the label.
            }
    ...
    for (int i = 0; i < PositionsTotal(); i++)
    {
        ...
        else
        {
            ...
            if ((ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
            {
                ...
                if ((be_line_color != clrNONE) && (BE_price > PositionGetDouble(POSITION_SL)))
                    DrawBELine((int)PositionGetInteger(POSITION_TICKET), BE_threshold, BE_price); // Only draw if not triggered yet.
            ...
            else if ((ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL)
            {
                double BE_threshold = NormalizeDouble(PositionGetDouble(POSITION_PRICE_OPEN) - sets.BreakEvenPoints * _Point, _Digits);
                double BE_price = NormalizeDouble(PositionGetDouble(POSITION_PRICE_OPEN) - extra_be_distance, _Digits);
                if ((be_line_color != clrNONE) && ((BE_price < PositionGetDouble(POSITION_SL)) || (PositionGetDouble(POSITION_SL) == 0)))
                    DrawBELine(PositionGetInteger(POSITION_TICKET), BE_threshold, BE_price);
...
double CalculateExistingVolume(bool all_symbols = true)
{
    ...
    if (AccountInfoInteger(ACCOUNT_MARGIN_MODE) == ACCOUNT_MARGIN_MODE_RETAIL_HEDGING) // Makes sense in a hedging mode.
    {
        for (int i = 0; i < total; i++)
        {
            ...
            if ((sets.MagicNumber != 0) && (PositionGetInteger(POSITION_MAGIC) != sets.MagicNumber))
                continue;
            if ((!all_symbols) && (PositionGetString(POSITION_SYMBOL) != SymbolForTrading))
                continue;
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        if ((sets.MagicNumber != 0) && (OrderGetInteger(ORDER_MAGIC) != sets.MagicNumber))
            continue;
        if ((!all_symbols) && (OrderGetString(ORDER_SYMBOL) != SymbolForTrading))
            continue;
...
void DrawBELine(ulong ticket, double be_threshold, double be_price)
{
    ...
    if (sets.ShowLines)
        ObjectSetInteger(ChartID(), obj_name, OBJPROP_TIMEFRAMES, OBJ_ALL_PERIODS);
    else
    ...
    if (ShowLineLabels)
    {
        ...
        if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
            text += TRANSLATION_MESSAGE_BUY;
        else if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL)
            text += TRANSLATION_MESSAGE_SELL;
